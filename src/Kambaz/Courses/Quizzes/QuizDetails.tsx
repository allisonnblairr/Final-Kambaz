...
export default function QuizDetails() {
  ...
  const reformatString = (isoString: any) => {
    ...
    const finalDate = `${date.toDateString()} at ${date.toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    })}`;
    return finalDate;
          ...
          >
            ...
            <div className="d-flex justify-content-center">
              <div className="quiz-details-list d-flex flex-column text-center my-4">
                ...
                {quiz.hasTimeLimit === true ? (
                  <div>
                    <strong>Time Limit:</strong> {quiz.timeLimitLength || "20"}{" "}
                    Minutes
                  </div>
                ) : (
                  <div>
                ...
                <div>
                  ...
                  {quiz.hasMultipleAttempts === true ? "Yes" : "No"}
                </div>
                <div>
                  <strong>Number of Attempts:</strong> {quiz.numAttempts || "1"}
                </div>
                <div>
                ...
                {quiz.showCorrectAnswers === true && (
                  <div>
                    <strong>When to Show Correct Answers:</strong> After{" "}
                    {quiz.whenToShowCorrectAnswers || "1"} Day(s)
                  </div>
                )}
                <div>
            ...
            <div className="date-table">
              <Table responsive>
                ...
                <tbody>
                  <tr>
                    <td>{reformatString(quiz.due)}</td>
                    <td>Everyone</td>
                    <td>{reformatString(quiz.availableFrom)}</td>
                    <td>{reformatString(quiz.availableUntil)}</td>
                  </tr>
                </tbody>
              </Table>
            </div>
